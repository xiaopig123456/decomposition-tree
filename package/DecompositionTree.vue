 ...
 * 分解树
 * @author: 朱言俊
 * @description 项目外包联系我，业务范围：App(uni-app)、小程序、web前端、nodejs、php
 * @email: xiaopig123456@qq.com
 * @website: https://www.zhuyanjun.com
 * @date: 2024/6/28
 */
  ...
  watchEffect(() => {
    ...
    (new Array(columnCount.value)).fill([]).forEach((column, ci: number) => {
      ...
      tempTreeData.forEach((item, index) => {
        _columnData[ci].push({
          ...excludeKeys(item, ['children']),
          _uid: 'item-' + ci + '-' + index,
          _col: ci,
          _index: index,
          _active: selectedItem.value?.[ci] === index
        })
      })
    ...
    nextTick(function () {
      // 显示位置
      const _columnHeight: number[] = [];
      columnData.value.forEach((column, ci) => {
        let colHeight = 0;
        for (let i = 0; i < column.length; i++) {
          colHeight += props.itemHeight + (i ? props.itemSpacing : 0);
        }
        _columnHeight.push(colHeight)
      })

      let viewHeight: number;
      if (props.height) {
        const viewStyle = window.getComputedStyle(decompositionTreeRef.value);
        viewHeight = decompositionTreeRef.value.offsetHeight
            - parseFloat(viewStyle.paddingTop)
            - parseFloat(viewStyle.paddingBottom)
            - parseFloat(viewStyle.borderTopWidth)
            - parseFloat(viewStyle.borderBottomWidth);
      } else {
        viewHeight = Math.max(..._columnHeight);
      }

      const _columnCntOffset: number[] = [];
      columnData.value.forEach((column, ci) => {
        let colHeight = _columnHeight[ci];
        ...
        columnData.value.forEach((column, ci) => {
          ...
          column.forEach((item, index) => {
            const lineObj = LeaderLine.setLine(itemLineMap.get(parentItem._uid).el, itemLineMap.get(item._uid).el, {
              color: item._active ? props.activeColor : props.color,
              size: 0.5,
              endPlug: 'disc',
            });
...
function onItemHandleClick(item: treeItem, gi: number, index: number) {
  if (selectedItem.value[gi] !== index) {
    onItemClick(item, gi, index)
    return;
  }
...
<template>
  <div class="decomposition-tree" :style="{height: props.height}">
    <div class="decomposition-tree-header">
      <slot name="header" :selectedItem="selectedItem" :columnData="columnData"></slot>
    </div>
    <div class="decomposition-tree-body" :style="bodyStyle" ref="decompositionTreeRef">
      <template v-for="(group, gi) in columnData" :key="gi">
        <div v-if="group?.length" class="decomposition-tree-column" :style="{ paddingTop: `${columnCntOffset[gi]}px` }">
          <div
            ...
            <div class="decomposition-tree-item-cnt"
                 :style="{ borderColor: item._active ? props.activeColor : props.color }"
                 @click="onItemClick(item, gi, index)">
              <slot name="item" :item="item" :col="gi" :index="index">{{ item.id }}</slot>
            </div>
            <div class="decomposition-tree-item-handle"
                 :class="selectedItem[gi] === index && hideChildrenIndex !== gi ? 'expand' : ''"
                 :style="{ backgroundColor: props.activeColor }" @click="onItemHandleClick(item, gi, index)">
