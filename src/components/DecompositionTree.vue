                ...
                columnData.value.forEach((column, ci) => {
                    ...
                    column.forEach((item, index) => {
                        const lineObj = LeaderLine.setLine(itemLineMap.get(parentItem._uid).el, itemLineMap.get(item._uid).el, {
                            color: item._active ? props.activeColor : 'rgb(224, 227, 235)',
                            size: 0.5,
                            endPlug: 'disc'
                        });
            ...
            <template v-for="(group, gi) in columnData" :key="gi">
                <div v-if="group?.length" class="decomposition-tree-column" :style="{ paddingTop: `${columnCntOffset[gi]}px` }">
                    <div
                        ...
                        <div class="decomposition-tree-item-cnt" :style="{ borderColor: item._active ? activeColor : '' }" @click="onItemClick(item, gi, index)">
                            <slot name="item" :item="item" :col="gi" :index="index">{{ item.id }}</slot>
                        </div>
                        <div class="decomposition-tree-item-handle" :class="selectedItem[gi] === index && hideChildrenIndex !== gi ? 'expand' : ''" :style="{ backgroundColor: activeColor }" @click="onItemHandleClick(item, gi, index)">
